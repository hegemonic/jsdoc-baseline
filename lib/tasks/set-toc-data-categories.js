...
export default class SetTocDataCategories extends Task {
  run(ctx) {
    ...
    for (const category of tocCategories) {
      ...
      if (doclets.length) {
        items = doclets.map((doclet) => ({
          href: linkManager.getUri(doclet.longname),
          id: doclet.longname,
          // TODO: Differentiate between multiple items with the same name.
          label: doclet.name,
        }));
        hrefs = items.map((item) => item.href);

        tocData.push({ category, hrefs, items });
      }
    }
