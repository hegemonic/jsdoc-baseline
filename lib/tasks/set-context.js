...
module.exports = class SetContext extends Task {
  constructor(opts) {
    ...
    this.func = (ctx) => {
      try {
        ...
        ctx.template = new Template(ctx.templateConfig, ctx.dependencies);
  ...
  _handleDoclet(doclet, ctx, workspace) {
    const { allDocletsByLongname, allLongnames, events, globals, listeners, needsOutputFile } =
      workspace;
  ...
  _updateContext(ctx, workspace) {
    ...
    ctx.navTree = name.longnamesToTree(Object.keys(workspace.needsOutputFile));
