...
const { listMethods } = require('./list-methods');
...
function basicLink(item, linkManager, opts = {}) {
    if (opts.linkText) {
        opts.linkText = escape(opts.linkText);
    }
...
exports.Filters = class Filters {
    constructor(template) {
        ...
        this._markdownParser = template.config.markdown ? markdown.getParser() : fakeMarkdown;
    }
    ...
    /**
     * Given one or more CSS class names, returns a string that can be used as the `class` attribute
     * of an HTML element.
     *
     * If a class name includes a leading exclamation point (`!`), it will always be included in the
     * `class` attribute, but without the leading `!`. The configuration setting `cssClassPrefix`
     * can override the exclamation point with a different character.
     *
     * All other class names will be omitted unless the user added the class names to a CSS class
     * mapping file. In that case, the CSS class name will be mapped to the user's requested value,
     * and that value will be added to the `class` attribute.
     *
     * @param {...string} args - The names of CSS classes to include.
     * @return {string} The formatted `class` attribute, including a leading space (for example,
     * ` class="foo bar"`).
     */
    cssClass(...args) {
        const cssClasses = this._template.cssClasses;
        const keys = args.slice();
        const mappedClasses = [];
        const prefix = this._template.config.cssClassPrefix;

        keys.forEach(key => {
            // If the name has the appropriate prefix, strip the prefix and unconditionally add the
            // CSS class.
            if (key && key.startsWith(prefix)) {
                mappedClasses.push(key.substr(prefix.length));
            }
            // Otherwise, only add the name if the user asked for it. (A falsy value doesn't count
            // as "asking for it.")
        ...
        return new SafeString(` class="${mappedClasses.join(' ')}"`);
    }
     ...
     */
    describeType(parsedType, format = 'simple', property = 'description') {
        const catharsisOptions = getCatharsisOptions(this._template);
        let description;

        if (!['extended', 'simple'].includes(format)) {
            throw new Error('The describeType filter accepts the options "simple" and ' +
                '"extended".');
        ...
        if (format === 'extended') {
            return _.get(description.extended, property);
        } else {
    ...
    generatedBy() {
        ...
        return new SafeString(this._template.translate('generatedBy', {
    ...
    labels(doclet) {
        ...
        labels = labels.map(text => ({
            // Add the appropriate prefix so the class isn't dropped.
            class: `${prefix}label-${text}`,
        ...
        // Finally, prepend the label for the doclet's kind, if applicable. We want this label to
        // come first.
        if (ENUMS.OUTPUT_FILE_KINDS.includes(doclet.kind)) {
            labels.unshift({
                // Add the appropriate prefix so the class isn't dropped.
                class: `${prefix}label-kind`,
    ...
    licenseLink(licenseId) {
        ...
        if ({}.hasOwnProperty.call(spdxLicenses, licenseId)) {
            result = new SafeString(basicLink(spdxLicenses[licenseId].url, this._linkManager, {
    ...
    link(longname, linkText, monospace) {
        ...
        return new SafeString(basicLink(longname, this._linkManager, {
            linkText: linkText || '',
            monospace
        }));
    }
    ...
    linkLongnameWithSignature(doclet, cssClass) {
        ...
        if (cssClass && cssClass.startsWith(prefix)) {
            cssClass = cssClass.substr(prefix.length);
        }
        ...
        return new SafeString(basicLink(doclet.longname, this._linkManager, {
            cssClass,
            linkText
        }));
    }
     ...
     */
    linkToLine({lineno, shortpath}, cssClass, options = {}) {
        let fragmentId;
        let linkText;
        ...
        return new SafeString(basicLink(shortpath, this._linkManager, {
            cssClass,
            fragmentId,
            linkText,
            monospace
    ...
    modifierText({nullable, variable, defaultvalue}, isEnum) {
        ...
        if (typeof defaultvalue !== 'undefined' && !isEnum) {
            descriptions.push(this._template.translate('tables.body.defaultValue', {
    ...
    resolveAuthorLinks(text) {
        ...
        if (text) {
            ...
            if (matches && matches.length === 3) {
                text = `<a href="mailto:${matches[2]}">${escape(matches[1])}</a>`;
            } else {
                text = escape(text);
            }
        }
    ...
    translatePageTitle(prefix, title, category) {
        return new SafeString(this._template.translate('pageTitle', {
