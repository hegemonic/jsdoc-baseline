...
exports.Filters = class Filters {
  constructor(template) {
    ...
    this._highlighter = getHighlighter(this._dependencies, template.cssClasses);
    this._template = template;
    this._linkManager = template.linkManager;
    this._markdownParser = template.config.markdown
      ? markdown.getParser(this._dependencies)
      : fakeMarkdown;
  }
  ...
  cssClass(...args) {
    ...
    keys.forEach((key) => {
      ...
      if (key && key.startsWith(prefix)) {
        mappedClasses.push(key.substring(prefix.length));
      }
   ...
   */
  highlight(code, lang) {
    return this._highlighter(code, lang);
  }
  ...
  highlightUnwrapped(code, lang) {
    return this._highlighter(code, lang, { omitWrapper: true });
  }
