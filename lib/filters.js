...
export class Filters {
  ...
  badges(doclet) {
    ...
    badges = badges
      .map((text) => ({
        class: `badge-${text}`,
        text: _.capitalize(this._template.translate(`badges.${text}`)),
      }))
      .sort((a, b) => {
        if (a.text > b.text) {
          return 1;
    ...
    if (ENUMS.OUTPUT_FILE_KINDS.includes(doclet.kind)) {
      badges.unshift({
        class: 'badge-kind',
        text: _.capitalize(this._template.translate(`kinds.${doclet.kind}`)),
      });
    }

    // Finally, update CSS classes as needed.
    badges.forEach((badge) => {
      badge.class = this._cssClassMap.mapClassname(badge.class);
    });
  ...
  static async create(template) {
    ...
    filters._highlighter = await getHighlighter(filters._env);
    filters._markdownParser = template.config.markdown
      ? await getMarkdownRenderer(filters._env)
      : fakeMarkdown;
   ...
   */
  describeType(parsedType, kwargs) {
    const codeTag = kwargs?.codeTag ?? 'code';
  ...
  generatedBy() {
    ...
    return new SafeString(
      this._template.translate('generatedBy', {
        version: jsdocVersion(this._env),
  ...
  highlight(code, lang, kwargs) {
    return new SafeString(
      this._highlighter(code, lang, {
        cssClassMap: this._cssClassMap,
  ...
  idForString(filename, str) {
    return this._linkManager.registerFragmentId(filename, str);
  }
    ...
    if (_.get(this._template.config, 'tables.nestedPropertyTables')) {
      items.forEach((item, i) => {
        ...
        if (parentItem && parentItem.name && item.name) {
          ...
          if (match) {
            // Clone the item, and manually clone a non-enumerable property that we need to keep.
