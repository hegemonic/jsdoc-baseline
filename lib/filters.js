...
function joinNameStack(stack) {
  let joined = '';

  for (const item of stack) {
    // Don't prepend a scope character to the start of the joined name.
    if (joined.length) {
      joined += item.scopePunc;
    }
    joined += item.name;
  }
...
function getDescendants(docletStore, item, descendants = [], nameStack = []) {
  const itemMembers = Array.from(docletStore.docletsByMemberof.get(item.longname) ?? []);

  for (let itemMember of itemMembers) {
    const scopePunc = name.SCOPE_TO_PUNC[itemMember.scope];

    if (nameStack.length) {
      itemMember = Doclet.clone(itemMember);
      itemMember.name = joinNameStack(nameStack) + scopePunc + itemMember.name;
    }
    descendants.push(itemMember);

    nameStack.push({ scopePunc, name: itemMember.name });
    getDescendants(docletStore, itemMember, descendants, nameStack);
    nameStack.pop();
  }
...
export class Filters {
  ...
  /**
   * Gets all of the descendants of a doclet.
   *
   * A descendant is any doclet that has the original doclet as an ancestor. For example, if the
   * original doclet's longname is `module:settings.Settings`, then doclets with any of these
   * longnames are descendants of the original doclet:
   *
   * + `module:settings.Settings#load`
   * + `module:settings.Settings.create`
   * + `module:settings.Settings.defaultValues.fileTypes`
   *
   * The `name` property of each descendant is modified to contain its namepath relative to the
   * original doclet. For example, if the original doclet's longname is `module:settings.Settings`,
   * and one of its descendants has the longname `module:settings.Settings.defaultValues.fileTypes`,
   * then the descendant's `name` property contains the value `defaultValues.fileTypes`. This change
   * affects a copy of the doclet; the original doclet is not changed.
   *
   * @param {module:@jsdoc/doclet.Doclet} item - The doclet whose descendants you want to find.
   * @returns {Array<module:@jsdoc/doclet.Doclet>} The descendants of the original doclet.
   */
  getDescendants(item) {
    return getDescendants(this.#template.context.docletStore, item);
  }

  /**
   * Checks whether an object has at least one of the specified properties.
   *
   * @param {Object} obj - The object to check.
   * @param {Array<string>} props - The properties to look for.
   * @return {boolean} `true` if the object has at least one of the specified properties; otherwise,
   * `false`.
   */
  hasOneOf(obj, props) {
    if (!_.isObject(obj)) {
  ...
  #getCatharsisOptions(codeTag) {
    this.#catharsisOptions ??= {
      ...
      jsdoc: true,
      htmlSafe: true,
      links: this.#template.linkManager.stringToLinkUrl,
    };
  ...
  groupDocletTree(items) {
    ...
    function visit(item) {
      Object.keys(item).forEach((key) => {
        ...
        if (currentItem.children && (!currentItem.doclet || currentItem.doclet.kind !== 'enum')) {
   ...
   */
  idForDoclet(filename, doclet) {
    const { fragmentId } = this.#linkManager.registerDoclet(doclet);

    return fragmentId ?? this.idForString(filename, doclet.name);
  }
   ...
   *
   * @param {string} filename - The file being generated.
   * @param {(string|handlebars.SafeString)} str - The suggested value for the ID.
   * @return {string} An `id` attribute that's unique within the file and safe to use as a fragment
   * ID.
   */
  idForString(filename, str) {
    return this.#linkManager.registerFragmentId(filename, String(str));
  }
  ...
  /**
   * Sets a global variable that's available to all templates.
   *
   * If you overwrite an existing variable, the template might behave unexpectedly. To help prevent
   * this issue, choose a variable name that's not likely to overlap with an existing variable.
   *
   * @param {string} varname - The variable's name.
   * @param {*} value - The variable's value.
   */
  setGlobal(varname, value) {
    this.#template.renderEnv.globals[varname] = value;
  }
   ...
   */
  url(longname) {
    return this.#linkManager.getUrl(longname);
  }
}
