...
PublishJob.prototype.generateTocData = function generateTocData(navTree, options) {
    ...
    function TocItem(item, children) {
        this.label = helper.linkto(item.longname, stripNamespace(item.name));
...
PublishJob.prototype.generateTutorials = function generateTutorials(tutorials) {
    this.getTutorialChildren(tutorials).forEach(function(child) {
        var title = stripNamespace(child.title);
        var tutorialData = {
...
PublishJob.prototype.generateIndex = function generateIndex(packages, readme, sortedDoclets) {
    var data = {
        docs: sortedDoclets,
...
PublishJob.prototype.generateByLongname = function generateByLongname(longname, doclets, members) {
    ...
    Object.keys(doclets).forEach(function(category) {
        ...
        data = {
            ...
            pageTitle: stripNamespace(name.shorten(longname).name)
        };
