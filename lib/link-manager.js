...
export class LinkManager {
  ...
  #slugifyFilename;
  #stringToFilename;
  #stringToFilenameWithLinkExtension;
  ...
  constructor(opts) {
    ...
    const self = this;
    const stringToFilename = (this.#stringToFilename = new Map());

    this.#config = opts.config;
    this.#filenameToString = new Map();
    // Facade for `this.#stringToFilename` that we can pass to Catharsis.
    this.#stringToFilenameWithLinkExtension = {
      get: (prop) => {
        let val = stringToFilename.get(prop);
        if (val) {
          val = val + self.linkExtension;
        }
  ...
  getUri(str) {
    ...
    uri = this.#filenameWithLinkExtension(filename);
    fragmentId = this.#longnameToFragmentId.get(str);
    if (fragmentId) {
      uri = `${uri}#${fragmentId}`;
    }
  ...
  get stringToLinkUri() {
    return this.#stringToFilenameWithLinkExtension;
  }
}
