...
export class LinkManager {
  ...
  constructor(opts) {
    ...
    this._fragmentIdSlugifiersByFilename = new Map();
    this._slugifyFactory = opts.slugifyFactory || slugifyWithCounter;
  ...
  _getFragmentIdForLongname(filename, longname, id) {
    const fragmentId = this._longnameToFragmentId.get(longname);

    if (!fragmentId && id) {
      id = this._slugifyFragmentIdForFile(filename, id);
  ...
  _getFragmentIdForString(filename, id) {
    if (id) {
      id = this._slugifyFragmentIdForFile(filename, id);
    }
  ...
  _slugifyFragmentIdForFile(filename, id) {
    const slugify =
      this._fragmentIdSlugifiersByFilename.get(filename) ?? slugifyFragmentIdFactory();

    this._fragmentIdSlugifiersByFilename.set(filename, slugify);
   ...
   * the link text if the longname or URI is not recognized.
   */
  createLink(str, opts = {}) {
    ow(str, ow.string);
    ow(opts, ow.object);
  ...
  getUri(str) {
    ...
    const filename = this._stringToFilename.get(str);
    let fragmentId;
    let uri;
    ...
    uri = this._filenameWithLinkExtension(filename);
    fragmentId = this._longnameToFragmentId.get(str);
    if (fragmentId) {
      uri = `${uri}#${fragmentId}`;
    }
  ...
  registerDoclet(doclet) {
    ...
    if (OUTPUT_FILE_KINDS.includes(doclet.kind) || isModuleExports(doclet)) {
      ...
      if (doclet.name !== doclet.longname || doclet.scope === GLOBAL_SCOPE) {
        ...
        fragmentId = this._getFragmentIdForLongname(filename, doclet.longname, fragmentId);
      }
    }
  ...
  registerFragmentId(filename, id) {
    ...
    return this._getFragmentIdForString(filename, id);
  }
