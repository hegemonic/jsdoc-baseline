...
export class LinkManager {
  ...
  getUrl(str) {
    ...
    if (!filename) {
      return this.#xrefs.get(str.replace(/^external:/, '')) ?? '';
    }
  ...
  /*
...

    {
      ...
      xrefs: {
        'My#longname': {
          url: 'https://example.com/whatevs',
        },
      },
    }
  */
  registerXrefs(xrefs) {
    for (const [longname, data] of Object.entries(xrefs)) {
      this.#xrefs.set(longname, data.url);
    }
  }
