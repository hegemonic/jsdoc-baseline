...
module.exports = class DocletHelper {
  ...
  _trackByLongname(doclet, category) {
    const longname = doclet.longname;

    if (!isGlobal(doclet) && longname) {
      // Track the doclet by its longname.
      this.longname[longname] = hasOwnProp.call(this.longname, longname)
        ? this.longname[longname]
        : new SymbolTracker();
      this.longname[longname].add(doclet, category);
    }
  ...
  _categorize(doclet) {
    ...
    if (!shouldTrack(doclet)) {
      ...
      this._trackByLongname(doclet, category);
    }
  ...
  addDoclets(taffyData) {
    ...
    helper.prune(taffyData, this.dependencies);
