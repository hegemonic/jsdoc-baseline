...
import { CATEGORIES, KIND_TO_CATEGORY } from './enums.js';
...
export default class DocletHelper {
  ...
  _trackByLongname(doclet) {
    ...
    if (!isGlobal(doclet) && longname) {
      ...
      if (!Object.hasOwn(this.longname, longname)) {
  ...
  addDoclets(taffyData) {
    ...
    doclets = this.allDoclets = doclets
      .prune()
      .sortOn(['name', 'longname', 'version', 'since'])
      .value();
