...
const { escapeExpression, Exception, SafeString } = require('handlebars');
...
function didSetMonospace(options) {
    return options && options.hash && typeof options.hash.monospace === 'boolean';
}
...
module.exports = template => {
    ...
    const nameGen = template.linkManager;

    function link(item, opts = {}) {
        if (opts.linkText) {
            opts.linkText = escapeExpression(opts.linkText);
        }

        return nameGen.createLink(item, opts);
    }
    ...
    return {
        ...
        /**
         * Given a longname, return HTML links for the longname's ancestors. The list of ancestors
         * includes trailing scope punctuation.
         *
         * For example, given the longname `foo.bar.Baz`, this method returns a string similar to
         * `<a href="foo.html">foo</a>.<a href="foo-bar.html">bar</a>.`. Note that this string
         ...
         * `monospace` hash argument to `true`. For example: `{{ancestors foo.bar monospace=true}}`
         *
         ...
         */
        ancestors(longname, cssClass, options) {
            let links;
            let monospace = false;
            ...
            if (didSetMonospace(options)) {
                monospace = options.hash.monospace;
            }
         ...
         */
        id(doclet) {
            return nameGen.registerDoclet(doclet).fragmentId;
        },
        ...
        keys(obj) {
            ...
            if (typeof obj !== 'object') {
                message = 'The {{keys}} helper requires an object, but your value\'s type is ' +
                    `"${typeof obj}".`;
         ...
         * website](https://spdx.org/). If the license ID is not a valid SPDX identifier, this
         * method returns the license ID.
         *
        ...
        licenseLink(licenseId) {
            if ({}.hasOwnProperty.call(spdxLicenses, licenseId)) {
                return link(spdxLicenses[licenseId].name, {
                    linkText: `https://spdx.org/licenses/${licenseId}`
                });
            }
         ...
         *
         * To control whether the link uses a fixed-width font, set the `monospace` hash argument to
         * `true` or `false`. For example: `{{link foo.bar monospace=true}}`
         *
         ...
         */
        link(longname, linkText, options) {
            let monospace;
            ...
            return new SafeString(link(longname, {
                linkText: linkText || '',
                monospace
            }));
        },
        ...
        linkLongnameWithSignature(doclet, cssClass) {
            ...
            return new SafeString(link(doclet.longname, {
                cssClass,
                linkText
            }));
        },
         ...
         * L10N resource file.
         *
         * By default, the link does not use a fixed-width font. To use a fixed-width font, set the
         * `monospace` hash argument to `true`. For example: `{{linkToLine meta monospace=true}}`
         *
        ...
        linkToLine({lineno, shortpath}, cssClass, options) {
            ...
            linkText = template.translate('linkToLine', {
                filepath: shortpath,
                lineno: lineno,
                items: lineno
            });

            return new SafeString(link(shortpath, {
                cssClass,
                fragmentId,
                linkText,
                monospace
        ...
        packageLink(packageInfo, cssClass) {
            ...
            return new SafeString(link('index', {
                cssClass,
                linkText
            }));
        },
        ...
        resolveAuthorLinks(text) {
            ...
            if (text) {
                ...
                if (matches && matches.length === 3) {
                    text = `<a href="mailto:${matches[2]}">${escapeExpression(matches[1])}</a>`;
                } else {
        ...
        /**
         * Find inline `{@link}` tags, and replace them with HTML links. This method also finds and
         * converts variations on these tags that are recognized by JSDoc, including `{@linkplain}`
         * and `{@linkcode}`.
         *
         ...
         */
        resolveLinks(text) {
            return new SafeString(nameGen.resolveInlineLinks(text));
        },
        ...
        // TODO: Redundant. Remove and replace with `translate`.
        /**
         ...
         */
        url(longname) {
            return new SafeString(nameGen.getUri(longname));
        },
