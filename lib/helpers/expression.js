...
function addLink(item, linkText, linkClass, fragmentId) {
    var htmlLink;
    var regExp;
...
function formatParams(params) {
    ...
    params.filter(function(param) {
        ...
        if (param.variable) {
            formattedParam = translate('params.repeatable', {
                param: formattedParam
            }).trim();
        }

        formattedParam = translate('params.joiner', {
            items: i,
            param: formattedParam
        }).trim();

        if (param.optional) {
            formattedParam = translate('params.optional', {
                param: formattedParam
            }).trim();
        }
    ...
    formatted = translate('params.all', {
        params: formatted
    });
...
function link(input, linkText, linkClass, fragmentId) {
    return addLink(input, linkText, linkClass, fragmentId);
}

function linkList(items, open, joiner, close) {
    var links = [];

    if (items.length > 1) {
        open = firstDefinedValue(open, translate('linkList.open'));
        joiner = firstDefinedValue(joiner, translate('linkList.joiner'));
        close = firstDefinedValue(close, translate('linkList.close'));
    } else {
        open = joiner = close = '';
    }
...
function parseType(type) {
    var catharsisOptions = getCatharsisOptions();
...
module.exports = {
    ...
    ancestors: function(longname, cssClass) {
        ...
        links = getAncestors(longname).map(function(ancestor) {
            ...
            return addLink(ancestor, stripNamespace(ancestor), cssClass);
        });

        return new SafeString(links.join(''));
    },
    config: function(key) {
        return _.getPath(config(), key);
    },
    ...
    formatParams: function(params) {
        return new SafeString(formatParams(params));
    },
    generatedBy: function() {
        var formatter = new global.Intl.DateTimeFormat(config().locale, {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
        });

        return new SafeString(translate('generatedBy', {
    ...
    linkToLine: function(docletMeta, linkClass) {
        ...
        return new SafeString(link(docletMeta.shortpath, translate('linkToLine', {
    ...
    linkLongnameWithSignature: function(doclet, cssClass) {
        ...
        linkText = stripNamespace(doclet.longname || '');
    ...
    modifierText: function(doclet) {
        ...
        if (doclet.nullable === true) {
            descriptions.push(translate('tables.body.nullable.long'));
        } else if (doclet.nullable === false) {
            descriptions.push(translate('tables.body.nonNullable.long'));
        }

        if (doclet.variable === true) {
            descriptions.push(translate('tables.body.repeatable.long'));
        }

        if (doclet.defaultValue && !doclet.isEnum) {
            descriptions.push(translate('tables.body.defaultValue', {
                valueString: translate('tables.body.defaultValueString', {
    ...
    packageLink: function(packageInfo, cssClass) {
        ...
        linkText = packageInfo.name || translate('brandDefault');
    ...
    returnTypes: function(returns) {
        ...
        return parseType(typeUnion(typeUnions));
    },
    ...
    translate: function(key, items) {
        return new SafeString(translate(key, items.length));
    },
    translateHeading: function(key, items) {
        return new SafeString(translate('headings.' + key,
    ...
    translatePageTitle: function(title, category) {
        ...
        return new SafeString(translate('pageTitle', {
            category: translate('headings.' + category),
            title: title
        }));
    },
    typeNames: function(doclet) {
        if (doclet && doclet.type && doclet.type.names && doclet.type.names.length) {
            return new SafeString(linkList(doclet.type.names));
        }

        return '';
    },
    typeUnion: function(types) {
        return parseType(typeUnion(types));
    }
};
