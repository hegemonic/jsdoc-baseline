...
module.exports = function(template) {
    return {
        ...
        // TODO: replace with new expression helper `continued`
        ifContinued: function(options) {
            var data = options.data;

            if (data.currentSection !== data.nextSectionAfterColumn) {
                return options.inverse(this);
            }

            return options.fn(this);
        },
        // TODO: replace with new expression helper `shouldHighlight`
        ifHighlightedType: function(typeName, options) {
            var HIGHLIGHTED_TYPES = ['array', 'object'];

            if (HIGHLIGHTED_TYPES.indexOf(typeName) === -1) {
                return options.inverse(this);
            }
