...
function loadYaml(filepath, finder) {
    var parsedObject;

    try {
        if (filepath) {
            parsedObject = yaml.load(finder.readFileSync(filepath, 'utf8'));
        }
    } catch (e) {
...
var Template = module.exports = function(conf) {
    ...
    this.config = conf;
    this.cssClasses = config.readJson(finders.cssClassMap.findFileSync('classmap.json'));
...
Template.prototype._handlebarsInit = function() {
    ...
    if (!precompiled) {
        ...
        precompiled.forEach(function(precomp) {
            precompiledMerged = _.defaults(precompiledMerged, precomp);
        });
...
Template.prototype._l10nInit = function() {
    this.l10nData = loadYaml(this.config.l10nFile, finders.l10n);
